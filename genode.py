...
#
# Genode is free software; you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation; either version 3 of the License, or
# any later version.
#
# Genode is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with Genode; If not, see <http://www.gnu.org/licenses/>.
...
class DPG_GRAPH:
    ...
    node_tree = None
    node_editor_tabs = []
    main_tab_bar_id = None

    def on_plugin_select_callback(self, sender, keyword, app_data):
        print ("Plugin Selected: %s" % (keyword))
        print ("Launch an editor window for that plugin")
        plugin = factory.get_plugin(keyword)
        self.launch_editor(plugin)
    ...
    def __init__(self, debug=False):
        ...
        with dpg.window(label=MAIN_TITLE, width=MAIN_SIZE[0], height=MAIN_SIZE[1], pos = [0, 0]) as self.main_window:
            ...
            with dpg.tab_bar(tag="main_bar") as self.main_tab_bar_id:
                with dpg.tab(label="Main View", tag="main_tab"):
                    dpg.add_text("Main")
                    with dpg.group(horizontal=False):
                        lb = dpg.add_listbox(factory.get_plugin_names(), label="Name of Plugins", width=200, callback=self.on_plugin_select_callback)
                        #dpg.add_selection_callback(self.on_plugin_select_callback)
    ...
    def launch_editor(self, plugin):
        print("Maint Tab Bar ID: %s" % str(self.main_tab_bar_id))
        #mb = dpg.get_item_label("main_bar")
        tid = dpg.add_tab(label=plugin.name(), parent=self.main_tab_bar_id)
        plugin.start(self, parent = tid)
        #with dpg.group(horizontal=False, parent=tid):
        #    with dpg.group(horizontal=True):
        #        dpg.add_button(label="Start Graph",  callback=self.on_start_graph)
        #        dpg.add_button(label="End Graph",    callback=self.on_end_graph)
        #        with dpg.node_editor(label=plugin.name(),
        #                    callback=self.on_link,
        #                    delink_callback=self.delete_link) as ne:
        #            pass
    '''
    Callbacks
    '''
    ...
    def on_drag(self, sender, data, user_data):
        ...
        ## If resizing the sender then resize the sender itself
        #height = dpg.get_item_height(sender)
        #width = dpg.get_item_width(sender)
        #dpg.set_item_height(sender, height - 40)
        #dpg.set_item_width(sender, width - 20)

        ## If moving a selected node, update the node's stores coordinates
        #if np.sum(np.abs(data)) > 0 and len(nodes_list) > 0:
        #    for sel in dpg.get_selected_nodes(node_editor=sender):
        #        for n in nodes_list:
        #            if n.id == sel:
        ...
        #if len(dpg.get_selected_nodes(node_editor=sender)) == 0 and dpg.is_key_down(17):
...
def main(argv):
    ...
    if args.debug:
        ...
        print ("Plugin Names: %s" % str(factory.get_plugin_names()))

    g = DPG_GRAPH(debug=args.debug)
    g.start()
